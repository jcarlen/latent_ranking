...
\item The effect of positions on expected edge weights is symmetric, affecting both edges in a dyad equally. Positions influence the model via the total dyad weight. To condition on the total weight, as in the quasi-Stigler model, effectively eliminates the value of estimated positions.
...
With the introduction of latent space network models, \citet{hoff02} developed a stochastic Markov chain Monte Carlo (MCMC) estimation algorithm. \citet{hoff03} added the capacity to fit random effects, and \citet{krivitskyetal09} extended the model further and refined the underlying algorithm. In this section we present an overview of the MCMC estimation described by those authors, adapted for the latent space rating model and embellished with details from the \pkg{latentnet} implementation \citep{latentnet_jss, latentnet}. We conclude by discussing previous use of quasi-Netwon estimation and tradeoffs between this strategy and MCMC.
...
%- widely used and accesible to a wide audience
 ...
 We conclude by discussing previous use of quasi-Netwon estimation and tradeoffs between this strategy and MCMC.
...
Direct optimization of latent-space models has been used previously. \citet{handcock07} compared a two-stage maximum likelihood method to MCMC estimation for a latent space cluster model. They found that the two-stage MLE gives a good match to the MCMC fit in terms of cluster membership and relative positions, but the clusters are more spread out. In that case, the MLE is fit to the model without clusters, so it cannot capture dependence between positions and cluster assignments. In addition, the network fit are binary, so they provide less information than valued networks when fitting positions. More typically, direct optimization is used as an initialization step of MCMC, as in \citet{hoff02} and \citep{latentnet}. Methods introduced to speed up estimation such as the variational Bayesian method of \citet{vblpcm10, vblpcm} or the case-control approximate likelihood method of \citet{raftery12}, treat Bayesian MCMC estimation as the benchmark for complexity. Yet these methods compromise the true likelihood function on behalf of speed. For instance, positions fit by the variational Bayesian algorithm also show greater within-cluster variance than MCMC estimates \citep{vblpcm10}.

Although the scaling of the adjacency matrix, $\mathcal{O}(n^2)$, is problematic for quasi-Networn estimation, the number of iterations required is drastically smaller than for MCMC. Below 
...
library(xtable)
summary_xtable_rank <- xtable(summarytable_rank, caption = "Comparison of journal rankings", digits = 0, label = "summarytable_rank")
...
To choose the latent space dimension of the model we consider estimates of the Bayesian information criterion (BIC), which is itself a form of estimate of the integrated likelihood for the data under the model. %P(Y) = \int P(\theta|Y)P(\theta)d\theta. 
...
\noindent where $\hat{\theta}$ is the maximum-likelihood parameter estimate, $d$ is the dimension of the parameter space, and $s$ is the effective sample size. In the case of the latent space rating model, the MCMC-generated maximum-likelihood estimate may not be completely optimal, the parameter dimension is constrained by prior distributions, and different effective sample sizes are used in estimating various parameters. As an alternative to BIC we consider the BIC for model selection (BICM) of \citet{BICM}.
...
The right-hand plot of Figure \ref{fig:residuals} displays Pearson residuals against fitted values for the two-dimensional model. There is heightened variation where fitted values are close to zero and residuals must be negative. Although these mostly correspond to small absolute differences, the standardized scale reveals a lack of fit. To acccount for this in future implementations we could add a small constant to the citiation data matrix to stabilize the model for low counts. The underlying assumption is that all statistics journals in the network have at least a small chance of exchanging citations. Outside the near-zero area, the model spread appears roughly constant given the amount of data across fitted values, and does not appear overdispersed.
...
\noindent The value of the statistic ranges from zero to one, achieving the maximum if the network is symmetric. The denominator controls for the influence of variation in the dyad totals. We also consider an unnormalized version with the denominator removed. Figure \ref{fig:structure_reciprocity_plot} shows simulated reciprocity statistics based on a MKL point estimate for the two-dimensional latent space model; the posterior parameter distribution of the two-dimensional latent space model; and the point estimate for the quasi-Stigler model. To simulate networks from the quasi-Stigler model we use a beta-binomial approximation with mean and variance matching the overdispersed binomial. The left panel shows reciprocity while the right panel shows unnormalized values. 
...
recplot1 = ggplot(rec.data, aes(x = reciprocity, color = model)) +
  ...
  theme(axis.text.x = element_text(size = 8),
        legend.position = c(.8,.9), legend.text = element_text(size = 7),
        legend.key = element_rect(fill = "transparent"),
        legend.background=element_rect(fill="transparent"),
        legend.title = element_blank()) +
  scale_color_manual(values = c(hue_pal()(3)[2], hue_pal()(8)[8], hue_pal()(3)[3])) +
...
We now turn to the comparison of estimation methods for the latent space model. We illustrate that quasi-Netwon optimization returns item rankings and positional estimates nearly identical to MCMC results in much less time. 
...
print(xtable(results.table, caption = "Estimation methods. The $ell$ function in the column titles indicates the log of the probability.", digits = 4, label = "results.table"), caption.placement = "top", sanitize.text.function=function(x){x})
...
Table \ref{results.table} compares the results for the two-dimensional Poisson latent space model by three different estimations techniques: 1) The limited-memory bounded quasi-Newton method implemented by the \code{optim} (QN L-BFGS-B). 2) MCMC estimation as implemented by \pkg{latentnet}; and 3) MCMC as in method two but with random initialization of sender and receiver coefficients and MDS initial positions scaled so that the maximum coordinate size is one. Note that this initialization is also employed for quasi-Netwon estimation. For MCMC chains we allowed a burn-in period of $500000$, and collected a sample of size $5000$ by storing every $500th$ draw. We checked for convergence and appropriate MCMC interval and burn-in values using the \code{mcmc.diagnostics} function of \pkg{latentnet}. The MCMC method of row two seems to converge based on diagnostic plots, but given given the higher-probably estimate resulting from a different initialization method, we see that it has in fact converged to a local maximum.

Timing of computations was in R 3.3.2 with a MacBookPro, 2.6 GHz Intel Core i5 processor with 8 GB 1600 MHz DDR3 memory without. The ``code'' column indicates if the main operations were performed in \proglang{R} or \proglang{C}. Although not employed here, \pkg{latentnet} does allow for parallel processing of multiple MCMC chains. The time estimates should be considered ballpark for each method, as the number of iterations for the quasi-Netwon methods and control parameters for the MCMC could be augmented to achieve some speed-up. However, the process of optimizing these would mostly likely negate the time benefits.

We present MKL estimates from MCMC methods because they have higher posterior and graph likelihood than other point estimates, such as the posterior mode. Results in previous sections have all drawn on the MKL estimates from the MCMC estimation with random initialization (row three). The MCMC methods are clearly much slower than the quasi-Newton methods due to the number of iterations required, although they have the advantage of returning a posterior sample. The much faster estimates from the quasi-Newton methods are very highly correlated with these results (column five).
...
<<journal_simulation_study_plots, eval = T, echo = F, cache = T, warning = F, fig.width = 6, fig.height = 5, fig.cap = 'Distribution of differences in ratings (receiver - sender) and total actor parameters (sender + receiver + intercept) based on 100 simulations' , dependson = c("journal_simulation_study_data")>>=
...
<< poisson_glm_offset, eval = T, echo = F, warning = F, message = F, fig.keep = "none", cache = T>>=
...
To more formally examine the accuracy of quasi-Newton estimation we conduct a simulation study. We generate 100 simulated citation networks from the MKL point estimate and fit each by QN L-BFGS-B. We then compare the differences in ratings (sender minus receiver coefficient) and total actor parameters (sender plus receiver coefficient plus intercept) between the data generating model and the simulation estimate. Although the zero-mean prior reduces the non-identificability of the random effects, in practice with diffuse priors there is slight fluctuation. To account for this when calculating differences in ratings, we center the sender and receiver estimates to mean zero. The results are presented in Figure \ref{journal_simulation_study_plots}. The top panel shows that the differences in ratings are all concentrated near zero, indicating that QN L-BFGS-B does reliably recreate the ``true'' ratings in this case. Slighlty larger variances occur for journals such StataJ which have fewer non-zero edges. The bottom panel shows the differences in total actor parameters. Deviations from zero in this panel indicate changes in positions relative to the data-generating model. Journals estimated to be closer on aggregate to other journals compensate with smaller random effects. Three journals are estimated to be significantly closer to the rest of the network. We observe in the network visualization they each is in the periphery of the network. Thus, there is a linear range for which their position is nearly identified with the total of their sender and reciever coefficients. This is not a problem with the estimation method but stems from underlying uncertainty model. However, it does not affect our estimated rankings, as we see in the top panel, not the overall interpretation of the network. That said, it we must remember that the positions of peripheral nodes are more variable than central ones. 

Although the QN L-BFGS-B do not return a sample of the posterior distribution of $\theta$ like MCMC estimation, we can still estimate the variance in ratings. One method for doing this is to fit a Poisson GLM to the data matrix with estimated positions as an offset, i.e the distance term in \ref{eq3} is treated as a constant. We implemented such a model in this case and found similar estimates of ratings uncertainty to those derived from the MCMC sample. The estimated standard deviations range from 0.22 to 0.29, with differences from the MCMC estimates ranging from about 0 to 0.013. 
...
print(xtable(movie.table, caption = "Estimation methods. The $ell$ function in the column titles indicates the log of the probability.", digits = 4, label = "movie.table"),
